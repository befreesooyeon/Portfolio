...
const slot = (v) => (v ? v : '');
const list = (arr) => (Array.isArray(arr) && arr.length ? arr.map(v => `<li>${v}</li>`).join('') : '');

// 비주얼 이미지 소스(visualImg가 우선, 없으면 image 사용)
...
<section class="project-visual">
    ...
    <div class="top">
        ...
        <img class="visual-image" src="${slot(visualSrc)}" alt="${slot(project.title)}">
        </div>` : ''}
    ...
    <div class="bottom">
        <div class="project-about-block"><h3 class="project-label">ABOUT</h3><p class="project-text">${slot(project.about)}</p></div>
        <div class="project-about-block"><h3 class="project-label">DATE</h3><p class="project-text">${slot(project.aboutDate || project.date)}</p></div>
        <div class="project-about-block"><h3 class="project-label">ROLE & CONTRIBUTION</h3><p class="project-text">${slot(project.aboutRole || project.role)}</p></div>
        <div class="project-about-block"><h3 class="project-label">CATEGORY</h3><p class="project-text">${slot(project.aboutCategory || project.category)}</p></div>
    </div>
    </div>
</section>
...
<section class="project-solution">
    ...
    <div class="info">
        ...
        ${project.solutionAddImg1 ? `<img class="add-image" src="${project.solutionAddImg1}" alt="${slot(project.title)}">` : ''}
        ${project.solutionAddImg2 ? `<img class="add-image" src="${project.solutionAddImg2}" alt="${slot(project.title)}">` : ''}
        ${project.solutionAddImg3 ? `<img class="add-image" src="${project.solutionAddImg3}" alt="${slot(project.title)}">` : ''}
        </div>` : ''}
    </div>
...
<section class="project-media">
    ...
    <div class="Box1">
        ${project.box1Imgs.map(src => `<div class="imgItem"><img src="${src}" alt="${slot(project.title)}"></div>`).join('')}
    </div>` : ''}

    ${project.box2Imgs?.length ? `
    <div class="Box2">
        ${project.box2Imgs.map(src => `<div class="imgItem"><img src="${src}" alt="${slot(project.title)}"></div>`).join('')}
    </div>` : ''}
    </div>` : ''}
    ...
    <div class="phoneM">
    ${project.phone1Img ? `<div class="phone1"><img src="${project.phone1Img}" alt="${slot(project.title)}"></div>` : ''}
    ${project.phone2Img ? `<div class="phone2"><img src="${project.phone2Img}" alt="${slot(project.title)}"></div>` : ''}
    ${project.phone3Img ? `<div class="phone3"><img src="${project.phone3Img}" alt="${slot(project.title)}"></div>` : ''}
    </div>` : ''}
    ...
    <div class="project-nav-prev">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41" fill="none">
            ...
            <defs><clipPath id="clip0_prev"><rect width="40" height="40" fill="white" transform="matrix(0 1 1 0 0 0.5)"/></clipPath></defs>
        </svg>
        </span>
        <div class="label">
    ...
    <div class="project-nav-next">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="40" height="41" viewBox="0 0 40 41" fill="none">
            ...
            <defs><clipPath id="clip0_next"><rect width="40" height="40" fill="white" transform="translate(40 0.5) rotate(90)"/></clipPath></defs>
        </svg>
        </span>
    </div>
    </nav>
